...
class DatabakController extends Controller {
    ...
    function __construct() {
       ...
       if(!D('admininfo')->islogin()){//未登录
			$this->error_top('您尚未登录，请先登录！',U('login/login'),3);
		}	
    }
	...
	public function backdb(){
		...
		if (!file_put_contents($fileName, $content, LOCK_EX)) {
			$this->error('写入文件失败,请检查磁盘空间或者权限!',U('databak/index'),3);
		}
		$this->success('备份数据库成功！',U('databak/index'),3);
	}
	//删除备份
	public function delbak() {
		$filepath=C("DB_BACKUP").I("name");
		@unlink($filepath);
		$this->success('删除备份成功！',U('databak/index'),3);
    }
	//备份下载
